...
Using {..alf-code..} any code can be run during parsing, this is like action, not abortable, so better have it as last rule...

    > A= a [42]
    > B= A { $1.$10.$100 42 }
    > a
    
Alf code can also be run inside a gen
directive. Then the top value is popped
and stringified.
