...
data atom(char* s, char* hp, size_t size) { char* p= hp; int l= strlen(s);
  //if (l<=6) return char6nan(s);
  while(*p){if(!strcmp(s,p))return BOX(0,2,p-hp);p+=strlen(p)+1+1+sizeof(data);}
  assert(size==-1 || p+l+2-hp < size);
  return *p=l, BOX(0, 2, strcpy(p+1, s)-hp-1);
}
